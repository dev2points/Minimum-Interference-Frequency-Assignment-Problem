...
def add_sb(solver, num_vertices, num_frequencies, var_map, crt_file = None):
    top = solver.nof_vars()
    frequency_var = {}
    for i in range(num_frequencies):
        top += 1
    ...
    for i in range(num_vertices):
        for j in range(num_frequencies):
            solver.add_clause([-var_map[i][j], frequency_var[j]])

    for i in range(num_frequencies - 1):
        solver.add_clause([-frequency_var[i + 1], frequency_var[i]])

    degree_frequency=[0*num_frequencies]
...
def solve_and_print_incremental(solver, var_map, rhs, lits, total_penalty, max_penalty, type):
    # if type not in ('incremental', 'assumptions', 'first'):
    #     raise ValueError("Type must be either 'incremental', 'assumptions', or 'first'")
    ...
    if type == 'first':
        ...
        for i in range(total_penalty + 1, lits.len() + 1):
            solver.add_clause(-lits[i-1], -rhs[i-1][total_penalty-1])
        status = solver.solve()
...
def main():
    ...
    add_sb(solver, vertices, num_frequencies, var_map)
    ...
    while total_penalty > 0 :
        ...
        assignment= solve_and_print_incremental(solver, var_map, rhs, penalty_var_map, total_penalty, max_penalty, sys.argv[6])
        if assignment is None:
            return
        total_penalty =  check_solution(dataset, assignment, vertices)
        print(f"Time: {time() - start_time}")
